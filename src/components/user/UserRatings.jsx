  ...
  const updateRatings = (updatedReview, professorId) => {
    const tempRatings = ratings.map((rating) =>
      rating.professor.id === professorId
        ? {
            ...rating,
            ratings: rating.ratings.map((review) =>
          ...
          <div>
            ...
            {ratings.length > 0 ? (
              <div>
                  ...
                  <div key={"myratings_" + index}>
                    <div className="flex mb-30 tablet-mb-40  position-relative">
                      <div
                        ...
                        className={`circle-rating  circle${index % 4}`}
                      >
                        {rating.professor.name
                          .split(" ")[0]
                          .charAt(0)
                          .toUpperCase()}
                        {rating.professor.name
                      ...
                      <div className="ml-24">
                        <h2
                          ...
                          className="cursor-pointer text-20 text-000000 text-weight-600 mb-6"
                        >
                          {rating.professor.name}
                        </h2>
                        <p className="text-14 text-weight-400 text-595959">
                          {rating.professor.department_name} .{" "}
                          {rating.professor.institute_name}
                        </p>
                        <p className="text-14 text-weight-400 text-595959 mb-18">
                          <span className="text-1F1F1F text-weight-600">
                            {rating.professor.percentage_take_again}&nbsp;
                          </span>{" "}
                          Take Again&nbsp;|&nbsp;
                          <span className="text-1F1F1F text-weight-600">
                            {rating.professor.percentage_love_teaching_style}
                            &nbsp;
                          </span>{" "}
                          Students loves teaching style
                        </p>
                      </div>
                      <div
                      ...
                      >
                        <Image
                          width={32}
                          height={108}
                          src="/user/myRatingLine.svg"
                          alt="myRatingLine"
                          className="desktop-display-rating-img"
                        />
                        <div
                    ...
                    <div className="pl-72 mobile-padding-left-0">
                      <Reviews
                        ...
                        professorId={rating.professor.id}
                      />
                    </div>
                ...
                {Number(ratingsData.page) < ratingsData.lastPage && (
